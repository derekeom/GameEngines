...
{
	Vector3 segmentStart = mOwner.GetPosition();
	segmentStart.z += mFootCastOffset;
	Vector3 segmentEnd = mOwner.GetPosition();
	segmentEnd.z -= mFootCastOffset;
	
	if (mOwner.GetGame().GetPhysWorld().SegmentCast(mOwner, segmentStart, segmentEnd, outComp, outPos)
		&& outComp->GetIsWalkable())
	{
		return true;
	}
